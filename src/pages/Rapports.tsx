...
export const Rapports = () => {
  ...
  const wasteReasonsData = useMemo(() => {
    ...
    const reasonLabels: Record<string, string> = {
      'MACHINE_SETUP': t.MACHINE_SETUP,
      'MATERIAL_DEFECT': t.MATERIAL_DEFECT,
      'CUTTING_ERROR': t.CUTTING_ERROR,
      'OPERATOR_ERROR': t.OPERATOR_ERROR,
      'PRODUCT_DEFECT': t.PRODUCT_DEFECT,
      'OTHER': t.OTHER
    };
  ...
  const machinePerformance = useMemo(() => {
    return machines.map(m => {
      ...
      const mOrderIds = mOrders.map(o => o.id);
      
      const mEntries = production_entries.filter(e => mOrderIds.includes(e.of_id));
      const mProd = mEntries.reduce((sum, e) => sum + (e.good_quantity || 0), 0);
      const mScrap = mEntries.reduce((sum, e) => sum + (e.scrap_quantity || 0), 0);
      const mTotal = mProd + mScrap;
      const scrapRate = mTotal > 0 ? ((mScrap / mTotal) * 100).toFixed(1) : '0.0';
      const efficiency = mTotal > 0 ? ((mProd / mTotal) * 100).toFixed(1) : '0.0';
  ...
  }, [machines, orders, production_entries]);
  ...
  const handleGenerateInsights = async () => {
    ...
    const data = {
      ...
      globalWasteRate,
      globalEfficiency,
      topWasteReasons: wasteReasonsData.slice(0, 3),
      machinePerformance
    };
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t.reports_title}</h1>
          <p className="text-sm text-slate-500 font-medium mt-1">{t.reports_subtitle}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t.reports_total_prod}</p>
          <p className="text-2xl font-black text-slate-900 mt-2">{totalGood.toLocaleString()} <span className="text-xs text-slate-500 font-normal">{t.units}</span></p>
        </div>
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t.reports_total_waste}</p>
          <p className="text-2xl font-black text-red-600 mt-2">{totalScrap.toLocaleString()} <span className="text-xs text-slate-500 font-normal">{t.units}</span></p>
        </div>
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t.reports_waste_rate}</p>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">{t.reports_global_oee}</p>
        ...
        <>
          ...
          <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-white border border-blue-100 rounded-xl p-6 relative overflow-hidden shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-lg">{t.reports_ai_title}</h3>
                <p className="text-xs text-slate-500">{t.reports_ai_subtitle}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white border border-slate-200 rounded-xl p-6 flex flex-col justify-between shadow-sm hover:border-blue-300 transition-all">
              <div>
                ...
                <h3 className="font-bold text-slate-900 text-lg mb-1">{t.reports_daily}</h3>
                <p className="text-slate-500 text-sm mb-6">{t.reports_daily_desc}</p>
              </div>
              <div className="flex gap-3 mt-auto">
                ...
                <button onClick={() => { toast.success('Téléchargement PDF...'); generatePdfReport('daily', { production_entries, machines }, 'download'); }} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-1.5">
                  <span className="material-symbols-outlined text-[18px]">download</span> {t.download}
                </button>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 flex flex-col justify-between shadow-sm hover:border-blue-300 transition-all">
              <div>
                ...
                <h3 className="font-bold text-slate-900 text-lg mb-1">{t.reports_weekly}</h3>
                <p className="text-slate-500 text-sm mb-6">{t.reports_weekly_desc}</p>
              </div>
              <div className="flex gap-3 mt-auto">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 flex flex-col justify-between shadow-sm hover:border-blue-300 transition-all">
              <div>
                ...
                <h3 className="font-bold text-slate-900 text-lg mb-1">{t.reports_monthly}</h3>
                <p className="text-slate-500 text-sm mb-6">{t.reports_monthly_desc}</p>
              </div>
              <div className="flex gap-3 mt-auto">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="font-bold text-slate-900 text-lg">{t.reports_trend_title}</h3>
                <p className="text-xs text-slate-500 mt-0.5">{t.reports_subtitle}</p>
              </div>
              <select 
                value={periodFilter} 
              ...
              >
                <option value="7d">7 {t.days || 'Derniers Jours'}</option>
                <option value="month">{t.month || 'Ce Mois'}</option>
                <option value="year">{t.year || 'Année en cours'}</option>
              </select>
            </div>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip />
                  <Area type="monotone" dataKey="production" name={t.good_quantity} stroke="#3b82f6" fillOpacity={1} fill="url(#colorProd)" />
                  <Area type="monotone" dataKey="scrap" name={t.scrap_quantity} stroke="#ef4444" fillOpacity={1} fill="url(#colorScrap)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {activeTab === 'waste' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
              <h3 className="font-bold text-slate-900 text-lg mb-1">{t.reports_pareto_title}</h3>
              <p className="text-xs text-slate-500 mb-6">{t.reports_subtitle}</p>
              ...
              <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={wasteReasonsData} layout="vertical" margin={{ top: 5, right: 30, left: 40, bottom: 5 }}>
                    ...
                    <Tooltip />
                    <Bar dataKey="value" name={t.scrap_quantity} radius={[0, 4, 4, 0]}>
            ...
            {/* Waste by Machine */}
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
              <h3 className="font-bold text-slate-900 text-lg mb-1">{t.reports_machines_title}</h3>
              <p className="text-xs text-slate-500 mb-6">{t.overview}</p>
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-slate-100 bg-slate-50/50">
              <h3 className="font-bold text-slate-900">{t.reports_machines_title}</h3>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    ...
                    <th className="p-4">{t.scrap_quantity}</th>
                    <th className="p-4">{t.waste_percentage}</th>
                    <th className="p-4">{t.efficiency}</th>
                  </tr>
                </thead>
