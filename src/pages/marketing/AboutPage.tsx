    ...
    <div className={`min-h-screen font-sans transition-colors duration-300 ${
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className={`lg:col-span-6 rounded-3xl overflow-hidden border shadow-2xl aspect-[16/10] ${
          ...
          <div className="lg:col-span-6 space-y-6">
            <h2 className={`text-2xl sm:text-4xl font-black tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            ...
            <p className={`text-sm sm:text-base leading-relaxed ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
              <strong className={theme === 'dark' ? 'text-white' : 'text-slate-950'}>FactoryFlow TN transforme cette dynamique :</strong> un système MES SaaS ultra-réactif, utilisable en atelier sur de simples tablettes Android/iPad, avec codes PIN ouvriers, synchronisation temps réel et tarifs transparents en Dinars Tunisiens (TND).
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className={`p-8 border rounded-3xl text-center space-y-2 ${
            ...
            <span className="text-4xl font-black text-blue-500 font-mono">100%</span>
            <p className={`text-sm font-black uppercase tracking-wider ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Atelier Connecté</p>
            <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Élimination intégrale du papier sur les postes machines</p>
          </div>
          <div className={`p-8 border rounded-3xl text-center space-y-2 ${
            theme === 'dark' ? 'bg-slate-900/60 border-slate-800' : 'bg-white border-slate-200 shadow-sm'
          }`}>
            <span className="text-4xl font-black text-emerald-500 font-mono">-32%</span>
            <p className={`text-sm font-black uppercase tracking-wider ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Rebuts & Pertes</p>
            <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Grâce à l'analyse Pareto instantanée et alertes pannes</p>
          </div>
          <div className={`p-8 border rounded-3xl text-center space-y-2 ${
            theme === 'dark' ? 'bg-slate-900/60 border-slate-800' : 'bg-white border-slate-200 shadow-sm'
          }`}>
            <span className="text-4xl font-black text-cyan-500 font-mono">&lt; 48h</span>
            <p className={`text-sm font-black uppercase tracking-wider ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Mise en Service</p>
            <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Déploiement immédiat sans modification d'infrastructure</p>
          </div>
        </div>

        {/* Tunisian Industrial Commitment */}
        <div className={`p-8 sm:p-12 rounded-3xl border space-y-6 text-center max-w-4xl mx-auto ${
          ...
          <h3 className={`text-2xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            Fabriqué en Tunisie, pour l'Industrie Tunisienne 🇹🇳
          </h3>
          <p className={`text-sm sm:text-base leading-relaxed ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
